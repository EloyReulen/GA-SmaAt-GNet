...
class ConvDS(nn.Module):
    def __init__(self,in_channels, out_channels, kernels_per_layer=1):
        ...
        self.conv = nn.Sequential(
            DepthwiseSeparableConv(in_channels, out_channels, kernel_size=3, kernels_per_layer=kernels_per_layer, padding=1),
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True)
        )
...
class UpDS_Simple(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True, kernels_per_layer=1):
        ...
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = ConvDS(in_channels, out_channels, kernels_per_layer=kernels_per_layer)
        else:
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = ConvDS(in_channels, out_channels, kernels_per_layer=kernels_per_layer)
