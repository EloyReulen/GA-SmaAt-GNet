...
class UNet_base(pl.LightningModule):
    ...
    def loss_variance(self,y_pred,y_var,y_true):
        # Code from https://github.com/pmorerio/dl-uncertainty/blob/master/aleatoric-uncertainty/model.py
        loss1 = torch.mean( torch.exp(-y_var) * torch.square(y_pred-y_true))
        loss2 = torch.mean(y_var)
    ...
    def training_step(self, batch, batchid):
        x, mask, y, _ = batch
        y_pred, y_var = self(x, mask)

        if batchid % 100 == 0:
            # log sampled images
            var = torch.exp(y_var)
            display_list = [x[0], y[0], y_pred[0], var[0]]
            title = ['Input Image', 'Ground Truth', 'Predicted Image']

            f, axarr = plt.subplots(4,12)
            f.set_figwidth(20)
            for i in range(4):
                for j in range(12):
        ...
        mse = self.loss_func(y_pred.squeeze(), y)
        loss = self.loss_variance(y_pred.squeeze(),y_var.squeeze(),y)
        # logs metrics for each training_step,
        # and the average across the epoch, to the progress bar and logger
        self.log("train_loss", loss, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        self.log("train_mse", mse*100, on_step=True, on_epoch=True, prog_bar=True, logger=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        mse = self.loss_func(y_pred.squeeze(), y)
        self.log("val_mse", mse * 100, prog_bar=True)
    
    def on_validation_epoch_end(self):
        plt.close("all")

class Precip_regression_base_gnet_aleatoric(UNet_base):
    @staticmethod
    def add_model_specific_args(parent_parser):
